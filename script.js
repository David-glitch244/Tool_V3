...
function generateAlgoTableHtml(selectedDate, h, groupCols) {
    ...
    const targets = [0, 15, 30, 45];
    const selectedRows = [];
    let remainingRows = [...hourRows];
    for (let target of targets) {
        let closest = null;
        let minDiff = Infinity;
        for (let entry of remainingRows) {
            const diff = Math.abs(entry.dateObj.getMinutes() - target);
            if (diff < minDiff) {
                minDiff = diff;
                closest = entry;
            } else if (diff === minDiff && entry.dateObj.getSeconds() < closest.dateObj.getSeconds()) {
                closest = entry;
            }
        }
        if (closest) {
            selectedRows.push(closest);
            remainingRows = remainingRows.filter(e => e !== closest);
        }
    }
    selectedRows.sort((a, b) => a.dateObj - b.dateObj);
    let hasData = selectedRows.length > 0;
...
function showAnalysis() {
    ...
    for (let dayIndex = 0; dayIndex < 7; dayIndex++) {
        tableHtml += `<tr><td>${dayIndex + 1}</td><td>${count8PerDay[dayIndex]}</td><td>${count7PerDay[dayIndex]}</td><td>${count6PerDay[dayIndex]}</td><td>${inspection}</td><td>${prioritization}</td></tr>`;
    }
